...
// ─── WhatsApp Envíos (API Oficial / Sin restricciones de tandas) ────────────
try {
  localStorage.removeItem('segucar_wa_tanda_count');
  localStorage.removeItem('segucar_wa_tanda_time');
} catch(e) {}
...
async function fetchStats() {
  try {
    ...
    state.esDiaNoHabil = stats.es_dia_no_habil || false;

    // Banner de día no hábil: advierte si hoy es finde o feriado
    const bannerNoHabil = getEl('bannerDiaNoHabil');
    if (bannerNoHabil) {
      ...
      <div class="row-actions">
        ...
        ` : (client.telefono && client.telefono.length >= 10) ? `
          ${(() => {
            ...
            const btnLabel = isWaSent ? 'Enviado ✅' : (isDiaNoHabil ? '🏖️ No hábil' : 'Enviar');
            const btnDisabled = isDiaNoHabil ? 'disabled title="Hoy es día no hábil (finde o feriado AR). Los envíos se reanudan el próximo día hábil."' : `onclick="triggerSmartWhatsApp(${client.id}, '${poliza ? poliza.operacion : ''}')"`;
            const btnStyle = isDiaNoHabil
              ? 'padding: 6px 14px; font-weight: 700; font-size: 0.78rem; border-radius: 8px; display: inline-flex; align-items: center; gap: 6px; margin: 0; opacity: 0.55; cursor: not-allowed; background: rgba(255,200,50,0.1); border: 1px solid rgba(255,200,50,0.3); color: #f1c40f;'
              : 'padding: 6px 14px; font-weight: 700; font-size: 0.78rem; border-radius: 8px; display: inline-flex; align-items: center; gap: 6px; margin: 0; box-shadow: 0 4px 12px rgba(37,211,102,0.3);';
            return `
              <button type="button" class="btn btn-sm ${isDiaNoHabil ? '' : btnClass} btn-smart-wa" data-cli="${client.id}" data-pol="${poliza ? poliza.operacion : ''}" style="${btnStyle}" ${btnDisabled}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="${isDiaNoHabil ? '#f1c40f' : 'white'}"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                <span>${btnLabel}</span>
              </button>
            `;
          })()}
...
async function sendWhatsApp(clientId, templateId, operacion, vehiculo, fechaVenc, patente) {
  const client = state.clients.find(c => c.id === clientId);
  ...
  const popoverB = getEl('globalWaPopover');
  if (popoverB) popoverB.classList.remove('active');

  // Verificar si estamos en Modo Oficial API
  try {
    const resCfg = await fetch('/api/whatsapp/config');
    const cfg = await resCfg.json();
  ...
  // Modo Manual (WhatsApp Web directo sin límites de tandas)
  const url = `https://web.whatsapp.com/send?phone=${phone}&text=${encodeURIComponent(msg)}`;
  window.open(url, '_blank');
  markWhatsAppAsSent(clientId, operacion);
  showToast('Abriendo WhatsApp Web...', 'success');
...
// ─── DESPACHO AUTOMÁTICO DIARIO 8:00 AM (FRONTEND) ───────────────────────────

let currentDespachoData = null;

async function fetchDespachoBadge() {
  try {
    const res = await fetch('/api/automation/pendientes-hoy');
    if (!res.ok) return;
    const data = await res.json();
    const countEl = getEl('navDespachoCount');
    if (countEl) {
      const count = data.pendientes ? data.pendientes.length : 0;
      countEl.textContent = count;
      if (count === 0) {
        countEl.style.background = '#2ed573';
        countEl.style.color = '#1a1d20';
        countEl.textContent = '0';
      } else {
        countEl.style.background = '#f39c12';
        countEl.style.color = '#1a1d20';
      }
    }
  } catch (e) {
    console.warn('Error fetching despacho badge:', e);
  }
}

async function openDespachoModal() {
  const modal = getEl('modalDespacho8AM');
  if (!modal) return;
  modal.style.display = 'flex';
  await refreshDespachoStatus();
}
...
async function refreshDespachoStatus() {
  const dateEl = getEl('despachoFechaEval');
  const statusEl = getEl('despachoEstadoHoy');
  const badgesGrid = getEl('despachoBadgesGrid');
  const tableBody = getEl('despachoTableBody');

  if (statusEl) statusEl.innerHTML = '<span style="color:#48cae4;">Consultando pendientes del día...</span>';
  if (tableBody) tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:18px; color:var(--text-secondary);">Cargando...</td></tr>';

  try {
    const res = await fetch('/api/automation/pendientes-hoy');
    const data = await res.json();
    currentDespachoData = data;

    if (dateEl) dateEl.textContent = data.fecha || '--';

    if (data.dia_no_habil) {
      if (statusEl) statusEl.innerHTML = '<span style="color:#f1c40f;">🏖️ Hoy es Domingo o Feriado (Día no hábil). No se realizan envíos automáticos.</span>';
      if (badgesGrid) badgesGrid.innerHTML = '';
      if (tableBody) tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:18px; color:#f1c40f;">Día no hábil. Los avisos se despacharán el próximo día hábil a las 8:00 AM.</td></tr>';
      return;
    }

    const pendientes = data.pendientes || [];
    const counts = {
      renovacion_7_dias: pendientes.filter(p => p.tipo === 'renovacion_7_dias').length,
      recordatorio_48hs: pendientes.filter(p => p.tipo === 'recordatorio_48hs').length,
      primer_aviso: pendientes.filter(p => p.tipo === 'primer_aviso').length,
      segundo_aviso: pendientes.filter(p => p.tipo === 'segundo_aviso').length
    };

    if (statusEl) {
      if (pendientes.length === 0) {
        statusEl.innerHTML = '<span style="color:#2ed573;">✅ Todos los avisos de hoy fueron procesados o no hay pendientes.</span>';
      } else {
        statusEl.innerHTML = `<span style="color:#f39c12;">⚡ ${pendientes.length} avisos listos para despachar hoy</span>`;
      }
    }

    if (badgesGrid) {
      badgesGrid.innerHTML = `
        <div style="background:rgba(243,156,18,0.1); border:1px solid rgba(243,156,18,0.3); border-radius:8px; padding:8px 12px; text-align:center;">
          <div style="font-size:0.75rem; color:#f39c12;">Total Pendientes</div>
          <div style="font-size:1.25rem; font-weight:800; color:#fff;">${pendientes.length}</div>
        </div>
        <div style="background:rgba(72,202,228,0.1); border:1px solid rgba(72,202,228,0.3); border-radius:8px; padding:8px 12px; text-align:center;">
          <div style="font-size:0.75rem; color:#48cae4;">Renovación (7d)</div>
          <div style="font-size:1.25rem; font-weight:800; color:#fff;">${counts.renovacion_7_dias}</div>
        </div>
        <div style="background:rgba(241,196,15,0.1); border:1px solid rgba(241,196,15,0.3); border-radius:8px; padding:8px 12px; text-align:center;">
          <div style="font-size:0.75rem; color:#f1c40f;">Recordatorio (48h)</div>
          <div style="font-size:1.25rem; font-weight:800; color:#fff;">${counts.recordatorio_48hs}</div>
        </div>
        <div style="background:rgba(230,126,34,0.1); border:1px solid rgba(230,126,34,0.3); border-radius:8px; padding:8px 12px; text-align:center;">
          <div style="font-size:0.75rem; color:#e67e22;">Primer Aviso (48h)</div>
          <div style="font-size:1.25rem; font-weight:800; color:#fff;">${counts.primer_aviso}</div>
        </div>
        <div style="background:rgba(231,76,60,0.1); border:1px solid rgba(231,76,60,0.3); border-radius:8px; padding:8px 12px; text-align:center;">
          <div style="font-size:0.75rem; color:#e74c3c;">Segundo Aviso (96h)</div>
          <div style="font-size:1.25rem; font-weight:800; color:#fff;">${counts.segundo_aviso}</div>
        </div>
      `;
    }
    ...
    if (tableBody) {
      if (pendientes.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:18px; color:#2ed573;">No hay destinatarios pendientes de envío para hoy.</td></tr>';
      } else {
        const typeBadgeMap = {
          'renovacion_7_dias': '<span class="badge" style="background:rgba(72,202,228,0.15); color:#48cae4; border:1px solid rgba(72,202,228,0.3);">📄 Renovación (7 días)</span>',
          'recordatorio_48hs': '<span class="badge" style="background:rgba(241,196,15,0.15); color:#f1c40f; border:1px solid rgba(241,196,15,0.3);">🟡 Recordatorio Preventivo</span>',
          'primer_aviso': '<span class="badge" style="background:rgba(230,126,34,0.15); color:#e67e22; border:1px solid rgba(230,126,34,0.3);">🟠 Primer Aviso (48 hs)</span>',
          'segundo_aviso': '<span class="badge" style="background:rgba(231,76,60,0.15); color:#e74c3c; border:1px solid rgba(231,76,60,0.3);">🔴 Segundo Aviso (96 hs)</span>'
        };

        tableBody.innerHTML = pendientes.map(p => `
          <tr style="border-bottom:1px solid rgba(255,255,255,0.05);">
            <td style="padding:8px 10px; font-weight:600; color:#fff;">${escapeQuotes(p.nombre || '')}</td>
            <td style="padding:8px 10px; font-family:monospace; color:var(--text-secondary);">${p.telefono || '-'}</td>
            <td style="padding:8px 10px;"><strong style="color:#48cae4;">${p.patente || p.operacion || '-'}</strong> <span style="font-size:0.75rem; color:var(--text-secondary);">(${p.operacion})</span></td>
            <td style="padding:8px 10px;">${typeBadgeMap[p.tipo] || p.tipo}</td>
          </tr>
        `).join('');
      }
    }
    ...
    if (statusEl) statusEl.innerHTML = `<span style="color:#ff4757;">Error consultando pendientes: ${e.message}</span>`;
  }
}

async function ejecutarDespachoManual(dryRun = false) {
  if (!currentDespachoData || !currentDespachoData.pendientes || currentDespachoData.pendientes.length === 0) {
    showToast('No hay pendientes para despachar hoy', 'info');
    return;
  }

  const count = currentDespachoData.pendientes.length;
  const modoTexto = dryRun ? 'SIMULACIÓN (Dry-Run)' : 'ENVÍO OFICIAL POR WHATSAPP API';
  const confirmMsg = dryRun 
    ? `¿Deseás SIMULAR el despacho de ${count} mensajes para hoy?\n\n(No enviará mensajes reales a los clientes).`
    : `⚠️ ATENCIÓN: Vas a disparar ${count} mensajes oficiales de WhatsApp API.\n\n¿Confirmás el envío masivo del lote del día?`;

  if (!confirm(confirmMsg)) return;

  const btnEnviar = getEl('btnDespachoEnviar');
  const btnSimular = getEl('btnDespachoSimular');
  const progressContainer = getEl('despachoProgressBarContainer');
  const progressLabel = getEl('despachoProgressLabel');
  const progressCount = getEl('despachoProgressCount');
  const progressBar = getEl('despachoProgressBar');

  if (btnEnviar) btnEnviar.disabled = true;
  if (btnSimular) btnSimular.disabled = true;
  if (progressContainer) progressContainer.style.display = 'block';
  if (progressLabel) progressLabel.textContent = dryRun ? 'Simulando despacho...' : 'Enviando mensajes por WhatsApp API...';
  if (progressBar) progressBar.style.width = '20%';

  showToast(`Iniciando ${modoTexto} (${count} destinatarios)...`, 'info');

  try {
    const res = await fetch('/api/automation/ejecutar-despacho', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ dry_run: dryRun, force: true, delay_ms: dryRun ? 50 : 2500 })
    });

    const data = await res.json();
    if (!res.ok) throw new Error(data.error || data.message || 'Error en despacho');

    if (progressBar) progressBar.style.width = '100%';
    if (progressCount) progressCount.textContent = `${data.enviados_count || 0} / ${count}`;

    const resumenMsg = dryRun
      ? `🧪 Simulación completada: ${data.enviados_count} simulados, ${data.omitidos_silenciados_count} silenciados por humano, ${data.omitidos_ya_contactados_count} ya contactados.`
      : `✅ Despacho completado: ${data.enviados_count} enviados exitosamente, ${data.errores_count || 0} errores.`;

    showToast(resumenMsg, data.errores_count > 0 ? 'warning' : 'success');
    alert(resumenMsg);
    ...
    alert('❌ Error al ejecutar el despacho: ' + err.message);
  } finally {
