      ...
      <div class="row-actions">
        ...
        ${(client.telefono && client.telefono.length >= 10) ? `
          ${(() => {
            ...
            return `
              <div class="btn-group-wa" style="display: inline-flex; align-items: center; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 12px rgba(37,211,102,0.3); vertical-align: middle;">
                <button type="button" class="btn btn-sm ${btnClass} btn-smart-wa" data-cli="${client.id}" data-pol="${poliza ? poliza.operacion : ''}" style="padding: 6px 12px; font-weight: 700; font-size: 0.78rem; border-top-right-radius: 0; border-bottom-right-radius: 0; border-right: 1px solid rgba(255,255,255,0.18); display: inline-flex; align-items: center; gap: 4px; margin: 0; transform: none; box-shadow: none;" onclick="triggerSmartWhatsApp(${client.id}, '${poliza ? poliza.operacion : ''}')" title="Envío Inteligente Auto-detectado a ${escapeQuotes(formattedName)}">
                ...
                <button type="button" class="btn btn-sm ${btnClass} dropdown-toggle-wa" style="padding: 6px 8px; border-top-left-radius: 0; border-bottom-left-radius: 0; margin: 0; transform: none; box-shadow: none;" onclick="showWaPopover(event, ${client.id}, '${poliza ? poliza.operacion : ''}', '${poliza ? escapeQuotes(poliza.vehiculo || '') : ''}', '${poliza ? (poliza.fecha_vencimiento || '') : ''}', '${poliza ? escapeQuotes(poliza.patente || '') : ''}')" title="Elegir otra plantilla">
                  ▼
                </button>
              </div>
            `;
