...
14. ❌ **NO usar rangos acumulados (0-7 días) en Aviso Renovación**: 'Aviso Renovación (7 Días)' es un disparo PUNTUAL a exactamente 7 días del vencimiento (`diff = 7` o `calDiff === 7`), análogo al Recordatorio 48 hs en Cobranzas (`calDiff === 2`). Los contratos que vencen en días distintos de 7 (ej. 0–6 o >7 días) sin mora vencida permanecen en `🟢 Contrato Vigente`.
...
21. ❌ **NO usar filtros de texto genéricos como `.includes('48 hs')` para bloquear recordatorios preventivos**: La plantilla de *Primer Aviso* incluye `"48 hs"` en su título oficial (`🟠 Cuota: Primer Aviso (Vencida hace 48 hs)`). Bloquear por `"48 hs"` anula el Primer Aviso en cuotas vencidas y causa que el sistema salte erróneamente a *Segundo Aviso (96 hs)*.
22. ❌ **NO usar `?poli={op}` ni filtrar filas por `/cuota/i` en `muestro-polizas.php`**: El parámetro oficial de consulta en NRE es `?prop={op}`. Las filas de datos de la tabla de cuotas no contienen la palabra "cuota" (solo el encabezado la contiene); se deben identificar las tablas por cabecera (`Saldo Cli` / `Vencimiento`) y parsear todas las filas de datos.
