...
if (fs.existsSync(seedPath) && !fs.existsSync(dbPath)) {
    try {
        ...
        console.log('✅ Base de datos inicial sembrada desde seed.db.');
    } catch (e) {
...
try {
    ...
    db.prepare("UPDATE plantillas SET nombre_meta = 'aviso_renovacion_poliza_vencida_v2', mensaje = ? WHERE tipo = 'poliza_vencida'").run(
        ...
        db.transaction(() => {
            for (const g of activas) {
                ...
                if (isRenovacion) {
                    ...
                    // ⚠️ REGLA: Si generó nueva operación por renovación, cuenta como EXITOSO en gestiones (sale de la lista de renovaciones pendientes).
                    // El dinero recuperado se suma $0.00 si aún no pagó, o el valor de la póliza si ya pagó.
                    if (hasNewOp) {
